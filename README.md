...
MyRAG 是一个面向个人可信资料的知识库问答系统，目标是降低学习资料、技术文档和通知公告的重复检索成本，同时缓解通用大模型回答来源不可控的问题。系统支持将 LeetCode 题解、技术学习资料、学校通知公告等内容统一解析、清洗、切分、向量化并写入本地知识库，再通过 RAG 检索增强生成，让大模型尽量基于用户自己的资料进行回答。

## 核心实现与效果

- 数据清洗与规范化：对 PDF、DOCX、TXT、HTML、XLSX 等多源文档解析结果进行控制字符过滤、特殊空白符归一化、连续空格/空行压缩和首尾空白清理，减少格式噪声和脏数据进入向量库。
- Parent-Child Chunking：小粒度 Child Chunk 进入向量库用于精准召回，命中后回溯 Parent Chunk 作为完整回答上下文，兼顾检索精度和上下文完整性；在项目 benchmark 中，上下文噪声降低 23.48%，回答质量提升 6.42%。
- BM25 + 向量混合检索 + RRF 重排：同时覆盖专业术语、代码关键字、配置项等精确匹配场景和语义相似问题召回；在强标识符 benchmark 中，命中率从 50.00% 提升到 75.00%，上下文噪声降低 15.65%，回答质量提升 43.55%。
- Qwen 结合上下文的问题改写：将口语化、指代不清或缺少背景的问题改写为独立完整查询，减少多轮对话中的检索偏差；在 Query Rewrite benchmark 中，命中率从 55.00% 提升到 100.00%，上下文噪声降低 51.68%，回答质量提升 24.42%。
- 多轮记忆与上下文压缩：维护会话历史用于理解追问和指代，当历史消息超过阈值时自动压缩长期上下文，保留关键信息，降低 Token 消耗和无关信息干扰。
